...
for sample in \`seq `expr $iBeg + 1` `expr $iEnd + 1`\`
  ...
  for MTBin in high low
    do
    cmsStage -f /store/user/`whoami`/muHadAnalysis_\${MTBin}MT_TTJets_${version}.root /data1/`whoami`/TTJets/analysis/
    cmsRm /store/user/`whoami`/muHadAnalysis_\${MTBin}MT_TTJets_${version}.root
  done
done
