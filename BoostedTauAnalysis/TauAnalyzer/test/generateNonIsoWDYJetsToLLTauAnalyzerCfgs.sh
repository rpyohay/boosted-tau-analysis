...
if [ $# -gt 3 ]
    then
    echo "Usage: ./generateNonIsoWDYJetsToLLTauAnalyzerCfgs.sh <version> <template cfg> [reweightOnly]"
    exit 0
fi
...
nonIsoReweightTauAnalyzerOutputFiles=( "${tauAnalyzerOutputFilePrefix}muHadNonIsoReweightAnalysis_NonIsoWDYJetsToLL_M-10To50_${version}.root" "${tauAnalyzerOutputFilePrefix}muHadNonIsoReweightAnalysis_NonIsoWDYJetsToLL_M-50_${version}.root" )
...
for file in \`ls -alh tauanalyzer*NonIsoWDYJetsToLL*.sh | grep -v all | awk '{ print \$9 }'\`
  do
...
cat <<EOF > submitNonIsoWDYJetsToLLAllTauAnalyzerJobs.sh
#!/bin/bash

for file in \`ls -alh tauanalyzer*NonIsoWDYJetsToLL*all*.sh | awk '{ print \$9 }'\`
  do
  jobName=\`echo \$file | sed -e "s%\(.*\)\.sh%\1%"\`
  bsub -q 1nd -J \$jobName < \$file
done
...
exit 0
EOF
chmod a+x submitNonIsoWDYJetsToLLAllTauAnalyzerJobs.sh
...
cat <<EOF > copyAllNonIsoWDYJetsToLLFromEOS.sh
#!/bin/bash
...
for sample in "10To50" "50"
  ...
  for MTBin in high low
    do
    cmsStage -f /store/user/`whoami`/muHadAnalysis_\${MTBin}MT_NonIsoWDYJetsToLL_M-\${sample}_${version}.root /data1/`whoami`/nonIsoWDYJetsToLL/analysis/
    cmsRm /store/user/`whoami`/muHadAnalysis_\${MTBin}MT_NonIsoWDYJetsToLL_M-\${sample}_${version}.root
  done
done

exit 0
EOF
chmod a+x copyAllNonIsoWDYJetsToLLFromEOS.sh
