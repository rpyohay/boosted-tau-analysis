...
sed -e "s%cd \(.*\)%cd v35%" submitAll.sh > submitAll.sh.tmp && mv submitAll.sh.tmp submitAll.sh

chmod a+x generateWh1TauAnalyzerCfg.sh generateDYJetsToLLTauAnalyzerCfgs.sh generateTTJetsTauAnalyzerCfgs.sh generateWNJetsToLNuTauAnalyzerCfgs.sh generateDataTauAnalyzerCfgs.sh submitAll.sh
