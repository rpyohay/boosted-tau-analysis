#
#	singleEventGenerator.sh
#
#	Michael Manighalam, Brenna Mockler, Daniel Coelho
#	August 7, 2013
#	Madgraph 5 version 1.5.11
#
#	Runs one madgraph process
#
#	Define desired process chain in process_card.dat
#
#	This Script uses create_root_macro.sh, MadGraph5, and ExRootAnalysis.
#
#	Run using the following command in bash: sh singleEventGenerator.sh.
#	
...
#Manual Overwrite	
	...
	gunzip -vf MadGraphEvents/$fname/Events/run_01/"events.lhe.gz" #unzips the newly generated .lhe file
	cp MadGraphEvents/$fname/Events/run_01/"events.lhe"	Output_singleEvent/"events_$fname.lhe" #renames the .lhe file to correspond with the h01 mass and a01 mass for the event
	
	#Converts the LHE file to a root file
	$pathExRoot/ExRootLHEFConverter	Output_singleEvent/"events_$fname.lhe" Output_singleEvent/"events_$fname.root"
...
#root_macro_compiler to get a root file of histograms from the .C file
	 cp ../RootMacros/"templateRootMacro.C" $fname.C
    sed -i "s/templateRootMacro/$fname/" $fname.C
    sed -i "s/mh01(\"$hmassC\")/mh01(\"$mh01\")/" $fname.C
    sed -i "s/ma01(\"$amassC\")/ma01(\"$ma01\")/" $fname.C
    sed -i "s/Histograms_h01/Histograms_Entries_""$nevents""_h01/" $fname.C
